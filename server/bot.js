  ...
  try {
    ...
    if (commandName === 'project') {
      if (sub === 'list') {
        ...
        if (!projects.length) {
          return interaction.reply({ content: '📂 No projects yet. Create one on the web app or with `/project create`.', ephemeral: true })
        }
      ...
      if (sub === 'create') {
        ...
        return interaction.reply(`✅ Project **${name}** created! Open the web app to manage it.`)
      }
    }
    ...
    if (commandName === 'task') {
      if (sub === 'list') {
        ...
        if (!tasks.length) return interaction.reply({ content: `📋 No tasks in **${project.name}** yet.`, ephemeral: true })
      ...
      if (sub === 'create') {
        ...
        return interaction.reply(`✅ Task created in **${project.name}**\nID: \`${id.slice(0, 8)}\` · ${priorityEmoji(priority)} **${title}**`)
      }
      ...
      if (sub === 'done') {
        ...
        if (!taskDoc) return interaction.reply({ content: `❌ Task \`${shortId}\` not found.`, ephemeral: true })
        await taskDoc.ref.update({ status: 'done', completedAt: new Date().toISOString() })
        return interaction.reply(`✅ Marked **${taskDoc.data().title}** as done!`)
      }
      ...
      if (sub === 'update') {
        ...
        return interaction.reply(`🔄 Updated **${taskDoc.data().title}** → **${label}**`)
      }
    }
    ...
    if (commandName === 'note') {
      if (sub === 'add') {
        ...
        return interaction.reply(`📝 Note saved!\n> ${content.slice(0, 100)}${content.length > 100 ? '…' : ''}`)
      }
      ...
      if (sub === 'list') {
        ...
        if (!notes.length) return interaction.reply({ content: '📝 No notes yet.', ephemeral: true })
